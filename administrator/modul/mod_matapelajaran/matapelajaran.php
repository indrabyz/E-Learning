...
// Tampil MataKuliah
  default:
    if ($_SESSION[leveluser]=='admin'){
      ...
      echo "<h2>Manajemen MataKuliah</h2><hr>
          <input class='btn btn-info' type=button value='Tambah MataKuliah' onclick=\"window.location.href='?module=matapelajaran&act=tambahmatapelajaran';\">";
          echo "<br><br><table id='table1' class='gtable sortable'><thead>
          <tr><th>No</th><th>Id Mapel</th><th>Nama</th><th>Kelas</th><th>Dosen</th><th>Deskripsi</th><th>Aksi</th></tr></thead>";
    $no=1;
  ...
  if (!empty($cek_mapel)){
    echo"<h2>MataKuliah Yang Anda Ampu</h2><hr>
    ...
    echo "</table>";
            ...
            echo "<script>window.alert('Tidak ada MataKuliah yang anda ampu, Kembali ke home untuk menambah MataKuliah yang diampu');
    ...
    elseif ($_SESSION[leveluser]=='siswa'){
        ...
        echo"<br><b class='judul'>Daftar MataKuliah di Kelas Anda</b><br><p class='garisbawah'></p>";
...
case "tambahmatapelajaran":
    if ($_SESSION[leveluser]=='admin'){
        echo "<form method=POST action='$aksi?module=matapelajaran&act=input_matapelajaran'>
          ...
          <dl class='inline'>
          <dt><label>id matakuliah</label></dt>     <dd>: <input type=text name='id_matapelajaran' size=10></dd>
         ...
         <dt><label>Dosen</label></dt>              <dd>: <select name='id_pengajar'>
                                                  <option value=0 selected>--pilih--</option>";
                                                  $tampil_pengajar=mysql_query("SELECT * FROM siswa where level='dosen' ORDER BY nama_lengkap");
                                                  while($p=mysql_fetch_array($tampil_pengajar)){
                                                  echo "<option value=$p[id_siswa]>$p[nama_lengkap]</option>";
                                                  }echo "</select></dd>
    ...
    elseif ($_SESSION[leveluser]=='dosen'){
        echo "<form method=POST name='form_materi' action='$aksi?module=matapelajaran&act=input_matapelajaran_pengajar'>          
          <fieldset>
          <legend>MataKuliah yang di ampu</legend>
          <dl class='inline'>
...
case "editmatapelajaran":
    if ($_SESSION[leveluser]=='admin'){
        ...
        $pengajar = mysql_query("SELECT * FROM siswa where level='dosen' and id_siswa = '$m[id_pengajar]'");
        ...
        echo "
          ...
          <fieldset>
          <legend>Edit MataKuliah</legend>
          <dl class='inline'>
          <dt><label>id matakuliah</label></dt>     <dd>: <input type=text name='id_matapelajaran' size=10 value='$m[id_matapelajaran]'></dd>
         ...
         <dt><label>Dosen</label></dt>              <dd>: <select name='id_pengajar'>
                                                  <option value='$d[id_siswa]' selected>$d[nama_lengkap]</option>";
                                                  $tampil_pengajar=mysql_query("SELECT * FROM siswa where level='dosen' ORDER BY nama_lengkap");
                                                  while($p=mysql_fetch_array($tampil_pengajar)){
                                                  echo "<option value=$p[id_siswa]>$p[nama_lengkap]</option>";
                                                  }echo "</select></dd>
        ...
        $kelas = mysql_query("SELECT * FROM kelas WHERE id_kelas = '$m[id_kelas]'");
        $k = mysql_fetch_array($kelas);
        $pengajar = mysql_query("SELECT * FROM siswa WHERE id_siswa = '$m[id_pengajar]' and level='dosen'");
        $d = mysql_fetch_array($pengajar);

        echo "<form method=POST name='form_materi' action='administrator/$aksi?module=matapelajaran&act=update_matapelajaran_pengajar'>
          <input type=hidden name=id value='$m[id]'>
          <fieldset>
          <legend>Edit MataKuliah</legend>
          <dl class='inline'>
...
case "detailpelajaran":
    if ($_SESSION[leveluser]=='admin'){
        ...
        echo "<div class='information msg'>Detail MataKuliah</div>
    ...
    while ($r=mysql_fetch_array($detail)){
       echo "<tr><td>$no</td>
             ...
             $pengajar = mysql_query("SELECT * FROM siswa where level='dosen' and id_siswa = '$r[id_pengajar]'");
             $cek_pengajar = mysql_num_rows($pengajar);
             if(!empty($cek_pengajar)){
             while($p=mysql_fetch_array($pengajar)){
             echo "<td><a href=?module=admin&act=detailpengajar&id=$r[id_pengajar] title='Detail Pengajar'>$p[nama_lengkap]</a></td>";
    ...
    <br><input class='btn btn-info' type=button value=Kembali onclick=self.history.back()>
      ...
      $detail =mysql_query("SELECT * FROM mata_pelajaran WHERE id_matapelajaran = '$_GET[id]'");
        echo "<span class='judulhead'><p class='garisbawah'>Detail MataKuliah</p></span>
    ...
    echo "</table>
    <input type=button value=Kembali class='btn btn-info' onclick=self.history.back()>";
    }
    break;
}
}
