  ...
  default:
    if ($_SESSION[leveluser]=='admin'){
      ...
      echo "<h2>Manajemen Kelas</h2><hr>
          <input type=button class='btn btn-info' value='Tambah Kelas' onclick=\"window.location.href='?module=kelas&act=tambahkelas';\">";
      echo "<br><br><table id='table1' class='gtable sortable'><thead>
          <tr><th>No</th><th>Id kelas</th><th>Kelas</th><th>Ketua Kelas</th><th>Aksi</th></tr></thead>";
    $no=1;
    while ($r=mysql_fetch_array($tampil)){       
       echo "<tr><td>$no</td>
             ...
             echo "<td><a href='?module=kelas&act=editkelas&id=$r[id]' title='Edit'><img src='images/icons/edit.png' alt='Edit' /></a> |
                 ...
                 <a href=?module=daftarsiswa&act=lihatmurid&id=$r[id_kelas]>Lihat MahaSiswa</a></td></tr>";
      $no++;
         ...
         if (!empty($ketemu)){
                ...
                while ($r=mysql_fetch_array($tampil_kelas)){
                    ...
                    echo "<td><a href='?module=kelas&act=editkelas&id=$r[id]' title='Edit'><img src='images/icons/edit.png' alt='Edit' /></a>|
                        ...
                        <input class='btn btn-info' type=button value='Lihat MahaSiswa' onclick=\"window.location.href='?module=daftarsiswa&act=lihatmurid&id=$r[id_kelas]';\">";
                $no++;
                }
                echo "</table>";
                }else{
    ...
    elseif ($_SESSION[leveluser]=='siswa'){
        ...
        echo "<table class='table table-condensed table-bordered table-hover'>
          <tr><th>No</th><th>Kelas</th><th>Ketua Kelas</th><th>Aksi</th></tr>";
        $no=1;
       ...
       echo "<tr>
             ...
             echo "<td><input type=button class='btn btn-small btn-info' value='Edit Kelas'
          onclick=\"window.location.href='?module=kelas&act=editkelas&id=$r[id]';\">
          <input type=button class='btn btn-small btn-warning' value='Lihat Teman'
    ...
    echo "<form method=POST action='$aksi?module=kelas&act=input_kelas'>
          ...
          <dt><label>Ketua Kelas</label></dt>     <dd> : <select name='id_siswa'>
                                      ...
                                      $tampil_siswa=mysql_query("SELECT * FROM siswa where level='siswa' ORDER BY nama_lengkap");
    ...
    elseif ($_SESSION[leveluser]=='dosen'){
        echo "<form name='form_kelas' method=POST action='administrator/$aksi?module=kelas&act=input_walikelas'>
          <fieldset>
    ...
    $tampil = mysql_query("SELECT * FROM kelas WHERE id = '$_GET[id]'");
    $r = mysql_fetch_array($tampil);
     echo "<form method=POST action='administrator/$aksi?module=kelas&act=update_walikelas'>
    <input type=hidden name=id value='$r[id]'>
          <fieldset>
              <legend>Edit Kelas</legend>
    ...
    elseif ($_SESSION[leveluser]=='siswa'){
        ...
        <tr><td colspan=2><input type=submit class='btn btn-small btn-info' value='Update'>
                          <input type=button class='btn btn-small btn-danger' value='Batal'
...
case "detailkelas":
    ...
    while ($r=mysql_fetch_array($detail)){
       echo "<tr>
             <td>$no</td>
             <td>$r[nama]</td>";
             $getpengajar = mysql_query("SELECT * FROM siswa WHERE id_siswa = '$r[id_pengajar]' and level='dosen'");
             $cek = mysql_num_rows($getpengajar);
             if (!empty($cek)){
             while($p=mysql_fetch_array($getpengajar)){
             echo "<td><a href=?module=admin&act=detailpengajar&id=$r[id_pengajar] title='Detail Wali Kelas'>$p[nama_lengkap]</a></td>";
             ...
             echo"<td><a href='?module=kelas&act=editkelas&id=$r[id]' title='Edit'> <img src='images/icons/edit.png' alt='Edit' /></a>|
                      <input type=button class='btn btn-info' value='Lihat MahaSiswa' onclick=\"window.location.href='?module=siswa&act=lihatmurid&id=$r[id_kelas]';\">";
       $no++;
      }
