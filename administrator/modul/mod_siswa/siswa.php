  ...
  default:
    if ($_SESSION[leveluser]=='admin'){
     ...
     $posisi = $p->cariPosisi($batas);
      ...
      $tampil_siswa = mysql_query("SELECT * FROM siswa where level='siswa' ORDER BY id_kelas LIMIT $posisi,$batas");
    ...
    while ($r=mysql_fetch_array($tampil_siswa)){
       echo "<tr><td>$no</td>
			 ...
			 <a href=javascript:confirmdelete('$aksi?module=siswa&act=hapussiswa&id=$r[id_siswa]') title='Hapus'><img src='images/icons/cross.png' alt='Delete' /></a> |
...
case "lihatmurid":
    ...
    if(!empty($cek_siswa)){
    echo "<div class='information msg'>Daftar MahaSiswa</div>
    ...
    while ($r=mysql_fetch_array($tampil)){
       echo "<tr><td>$no</td>
             ...
             <td><p align='center'>$r[blokir]</p></td>
             <td><a href='?module=siswa&act=editsiswa&id=$r[id_siswa]' title='Edit'><img src='images/icons/edit.png' alt='Edit' /></a> |
                 <a href=?module=detailsiswa&act=detailsiswa&id=$r[id_siswa]>detail mahahiswa</a></td></tr>";
      $no++;
    }
    echo "</table>";
    ...
    while ($r=mysql_fetch_array($tampil)){
       echo "<tr><td>$no</td>
             ...
             echo "<td><p align='center'>$r[jenis_kelamin]</p></td>                       
             <td><input type=button class='btn btn-info' value='detail mahahiswa' onclick=\"window.location.href='?module=detailsiswapengajar&act=detailsiswa&id=$r[id_siswa]';\">";
      $no++;
    }
    echo "</table>";
    $jmldata=mysql_num_rows(mysql_query("SELECT * FROM siswa"));
    ...
    while ($r=mysql_fetch_array($tampil)){
       echo "<tr><td>$no</td>
             ...
             <td>$r[th_masuk]</td>
             <td><input type=button class='btn btn-small btn-info' value='detail mahahiswa'
...
case "tambahsiswa":
  ...
  case "editsiswa":
    ...
    echo "<tr><td>Jabatan</td>      <td> : <input type=text name='jabatan' size=70 value='$r[jabatan]'></td></tr>
          <tr><td colspan=2><input type=submit class='btn btn-small btn-info' value='Update'>
                            <input type=button class='btn btn-small btn-info' value='Batal'
                            onclick=self.history.back()>
                            </td></tr>
          </table></form>";
    }
 ...
 case "detailsiswa":
    if ($_SESSION[leveluser]=='admin'){
       ...
       echo "<form><fieldset>
          <legend>detail mahahiswa</legend>
          <dl class='inline'>
    ...
    elseif ($_SESSION[leveluser]=='dosen'){
       ...
       <table id='table1' class='table table-condensed table-bordered table-hover'>
          ...
          <tr><td>Aksi</td>               <td> : <input type=button class='btn btn-info' value=Kembali onclick=self.history.back()></td></tr>";
    ...
    elseif ($_SESSION[leveluser]=='siswa'){
      ...
      echo"<br><b class='judul'>Detail Mahasiswa</b><br><p class='garisbawah'></p>
       <table class='table table-condensed table-bordered table-hover'>
          ...
          <tr><td>E-Mail</td>             <td> : <a href=mailto:$siswa[email]>$siswa[email]</a></td></tr>
          <tr><td>No.Telp/Hp</td>         <td> : $siswa[no_telp]</td></tr>";
          echo"<tr><td colspan='3'><input type=button class='btn btn-small btn-info' value='Kembali'
...
case "detailprofilsiswa":
    if ($_SESSION[leveluser]=='siswa'){
      ...
      echo"<br><b class='judul'>detail mahahiswa</b><br><p class='garisbawah'></p>
       <table class='table table-condensed table-bordered table-hover'>
             <tr><td rowspan='14'>";if ($siswa[foto]!=''){
              echo "<img src='foto_siswa/medium_$siswa[foto]'>";
          }echo "</td><td>Nis</td>        <td> : $siswa[nis]</td></tr>
          ...
          <tr><td>No.Telp/Hp</td>         <td> : $siswa[no_telp]</td></tr>
          <tr><td>Jabatan</td>            <td> : $siswa[jabatan]</td></tr>";
          echo"<tr><td colspan='3'><input type=button class='btn btn-small btn-info' value='Edit Profil' onclick=\"window.location.href='?module=siswa&act=editsiswa&id=$siswa[id_siswa]';\"></td></tr></table>";
    }
    break;
...
case "detailaccount":
    if ($_SESSION[leveluser]=='siswa'){
        ...
        <tr><td colspan=2><input type=submit class='btn btn-small btn-info' value='Update'></td></tr>
        </table>";
    }
    break;
}
