...
switch($_GET[act]){
 ...
 elseif ($_SESSION[leveluser]=='dosen'){        
    ...
    $cek_materi = mysql_query("SELECT * FROM file_materi WHERE pembuat = '$_SESSION[idsiswa]'");
    
     echo "<h2>Daftar Materi Yang Anda Upload</h2><hr>
          <input class='btn btn-info' type=button value='Tambah Materi' onclick=\"window.location.href='?module=materi&act=tambahmateri';\">";
    ...
    while ($r=mysql_fetch_array($cek_materi)){
      $tgl_posting   = tgl_indo($r[tgl_posting]);
       echo "<tr><td>$no</td>
             ...
             <td>$r[hits]</td>
             <td><a href='?module=materi&act=editmateri&id=$r[id_file]' title='Edit'><img src='images/icons/edit.png' alt='Edit' /></a>  |
            <a href=javascript:confirmdelete('administrator/$aksi?module=materi&act=hapus&id=$r[id_file]') title='Hapus'><img src='images/icons/cross.png' alt='Delete' /></a></td></tr>";
    ...
    echo"</table>";   
       ...
       echo "<table class='table table-condensed table-bordered table-hover'>
          <tr><th>No</th><th>MataKuliah</th><th>Materi</th></tr>";
        $no=1;
        while ($r=mysql_fetch_array($mapel)){
        echo "<tr><td>$no</td>
             <td>$r[nama]</td>";
             echo "<td><input type=button class='btn btn-small btn-info' value='Lihat File Materi'
...
case "daftarmateri":
    if ($_SESSION[leveluser] == 'siswa'){
        ...
        echo "<tr><td rowspan='5'>$no</td>";
             ...
             <tr><td colspan=2><input type=button class='btn btn-small btn-info' value='Download File'
        ...
        echo "<p class='garisbawah'></p><input type=button class='btn btn-small btn-danger' value='Kembali'
          onclick=self.history.back()>";
    }
    else{
        echo "<script>window.alert('Tidak ada file materi di MataKuliah ini?');
...
case "tambahmateri":
    ...
    echo "
    <form name='form_materi_pengajar' method=POST action='administrator/$aksi?module=materi&act=input_materi' enctype='multipart/form-data'>
    <fieldset>
    <legend>Tambah Materi</legend>
    <dl class='inline'>
    <dt><label>Judul</label></dt>              <dd> <input type=text name='judul' size=50></dd>
    ...
    <br /><p><input class='btn btn-info' type=submit value=Simpan>
                      <input class='btn btn-warning' type=button value=Batal onclick=\"window.location.href='?module=materi';\"></p>
    </dl></fieldset></form>";
    }
    break;
...
case "editmateri":
    ...
    echo "<form name='form_materi_pengajar' method=POST action='administrator/$aksi?module=materi&act=edit_materi' enctype='multipart/form-data'>
    <input type=hidden name=id value='$m[id_file]'>
    <fieldset>
    <legend>Edit Materi</legend>
    <dl class='inline'>
    ...
    <dt><label>Kelas</label></dt>              <dd>: <select name='id_kelas' onChange='showpel_pengajar()'>
                                          <option value='".$k[id_kelas]."' selected>".$k[nama]."</option>";
                                          $pilih="SELECT * FROM kelas WHERE id_pengajar = '$_SESSION[idsiswa]'";
                                          $query=mysql_query($pilih);
                                          while($row=mysql_fetch_array($query)){
                                          echo"<option value='".$row[id_kelas]."'>".$row[nama]."</option>";
                                          }
    ...
    <small>Apabila file tidak diganti, di kosongkan saja</small></dd>
    <br /><p><input class='btn btn-info' type=submit value=Simpan>
                      <input class='btn btn-warning' type=button value=Batal onclick=self.history.back()></p>
    </dl></fieldset></form>";
    }
    break;
