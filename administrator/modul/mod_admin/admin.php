...
switch($_GET[act]){
  ...
  case "pengajar":
  if ($_SESSION[leveluser]=='admin'){
      $tampil_pengajar = mysql_query("SELECT * FROM siswa where level='dosen' ORDER BY username_login");
    echo "<h2>Manajemen Dosen</h2><hr>
          <input class='btn btn-info' type=button value='Tambah Dosen' onclick=\"window.location.href='?module=admin&act=tambahpengajar';\">";
    ...
    while ($r=mysql_fetch_array($tampil_pengajar)){
       echo "<tr><td>$no</td>
             ...
             <td>$r[nama_lengkap]</td>             
		         <td align=center>$r[blokir]</td>
             <td><a href='?module=admin&act=editpengajar&id=$r[id_siswa]' title='Edit'><img src='images/icons/edit.png' alt='Edit' /></a> |
			 <a href=javascript:confirmdelete('$aksi?module=admin&act=hapusdosen&id=$r[id_siswa]') title='Hapus'><img src='images/icons/cross.png' alt='Delete' /></a> |
                 <a href=?module=detailpengajar&act=detailpengajar&id=$r[id_siswa]>Detail</a></td></tr>";
      $no++;
    }
    echo "</table>";
  }else{
 ...
 case "editpengajar":
    ...
    if ($_SESSION[leveluser]=='admin'){
    echo "<form method=POST action=$aksi?module=admin&act=update_pengajar enctype='multipart/form-data' id='contact-form' class='form'>
          <input type=hidden name=id value='$r[id_siswa]'>
          <fieldset>
          <legend>Edit Dosen</legend>
          <dl class='inline'>
          <dt><label>Nip</label></dt>          <dd> : <input type=text name='nip' value='$r[nis]'></dd>
    ...
    elseif ($_SESSION[leveluser]=='dosen'){
        $edit=mysql_query("SELECT * FROM siswa WHERE id_siswa='$_SESSION[idsiswa]'");
        $r=mysql_fetch_array($edit);
     echo "<form method=POST action=administrator/$aksi?module=admin&act=update_pengajar2 enctype='multipart/form-data' class='table table-condensed table-bordered table-hover'>
          <input type=hidden name=id value='$r[id_siswa]'>          
          <fieldset>
          <legend>Edit Profil</legend>
         </td></tr></table>
		  <dl class='inline'>
          <dt><label>Nip</label></dt>          <dd> : <input type=text name='nip' value='$r[nis]'></dd></td></tr></table>
          <dt><label>Nama Lengkap</label></dt> <dd> : <input type=text name='nama_lengkap' size=30 value='$r[nama_lengkap]'></dd>
          <dt><label>Username</label></dt>     <dd> : <input type=text name='username' value='$r[username_login]'></dd>
          <dt><label>Password</label></dt>     <dd> : <input type=text name='password'>
                                                      <small>Apabila password tidak diubah, dikosongkan saja</small>
...
case "detailpengajar":
    ...
    echo "<form><fieldset>
          ...
          <dl class='inline'>
          <dt><label>Nip</label></dt>   <dd> : $r[nis]</dd>
    ...
    elseif ($_SESSION[leveluser]=='dosen'){
        echo "<form><fieldset>
          ...
          }echo "</td><td>Nip</td>  <td> : $r[nis]</td><tr>
        ...
        echo "<table class='table table-condensed table-bordered table-hover'>
          ...
          <tr><td>E-mail</td>       <td> : <a href=mailto:$r[email]>$r[email]</a></td></tr>
          <tr><td>Jabatan</td>      <td> : $r[jabatan]</td></tr>
          <tr><td colspan='3'><input type=button class='btn btn-small btn-danger' value='Kembali'
