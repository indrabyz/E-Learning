...
<script language="JavaScript" type="text/JavaScript">
 ...
 while ($data = mysql_fetch_array($hasil))
 {
   ...
   // membuat option mata pelajaran untuk masing-masing kelas
   $query2 = "SELECT * FROM mata_pelajaran WHERE id_kelas = '$idkelas' AND id_pengajar ='$_SESSION[idsiswa]'";
   $hasil2 = mysql_query($query2);
   $content = "document.getElementById('pelajaran').innerHTML = \"<select name='".id_matapelajaran."'>";
   while ($data2 = mysql_fetch_array($hasil2))
   {
  ...
  default:
      ...
      if ($_SESSION[leveluser]=='admin'){    
        echo "<h2>Manajemen Tugas/Quiz</h2><hr>
          <input type=button class='btn btn-info' value='Tambah Topik' onclick=\"window.location.href='?module=quiz&act=tambahtopikquiz';\">";
      ...
      $tgl_buat   = tgl_indo($r[tgl_buat]);
       echo "<tr><td>$no</td>
             ...
             <td><ul><li><a href='?module=quiz&act=edittopikquiz&id=$r[id_tq]' title='Edit'><img src='images/icons/edit.png' alt='Edit' /></a> | 
                 ...
                 <li><a href=?module=buatquiz&act=buatquiz&id=$r[id_tq] class='btn btn-info'>Buat Quiz</a></li>
                 <li><a href=?module=daftarquiz&act=daftarquiz&id=$r[id_tq] class='btn btn-warning'>Daftar Quiz</a></li>
                 <li><a href=?module=quiz&act=daftarsiswayangtelahmengerjakan&id=$r[id_tq] class='btn btn-danger'>Daftar Peserta & Koreksi</a></li></ul></td></tr>";
      $no++;
    }
    echo "</table>";
    ...
    $tampil_topik = mysql_query("SELECT * FROM topik_quiz WHERE pembuat = '$_SESSION[idsiswa]'");
        echo "<h2>Daftar Topik Quiz</h2><hr>
          <input class='btn btn-info' type=button value='Tambah Topik' onclick=\"window.location.href='?module=quiz&act=tambahtopikquiz';\">";
        ...
        echo "<tr><td>$no</td>
             ...
             <td><a href='?module=quiz&act=edittopikquiz&id=$r[id_tq]' title='Edit'><img src='images/icons/edit.png' alt='Edit' /> | 
                         <a href=javascript:confirmdelete('administrator/$aksi?module=quiz&act=hapustopikquiz&id=$r[id_tq]') title='Hapus'><img src='images/icons/cross.png' alt='Delete' /></a><br><br>
                 <input type=button class='btn btn-info' style='margin:1px;' value='Buat Quiz' onclick=\"window.location.href='?module=buatquiz&act=buatquiz&id=$r[id_tq]';\"></input><br>
                <input type=button class='btn btn-warning' style='margin:1px;' value='Daftar Quiz' onclick=\"window.location.href='?module=daftarquiz&act=daftarquiz&id=$r[id_tq]';\"></input><br>
                <input type=button class='btn btn-danger' style='margin:1px;' value='Peserta & koreksi' onclick=\"window.location.href='?module=quiz&act=daftarsiswayangtelahmengerjakan&id=$r[id_tq]';\"></input></td></tr>";
      $no++;
    }
    echo "</table>";      
    }
        ...
        if (!empty($cek_mapel)){
            ...
            while ($t=mysql_fetch_array($mapel)){                
                echo "<tr><td>$no</td>
                        ...
                        echo"<td><input type=button class='btn btn-small btn-info' value='Lihat Tugas / Quiz'
...
case "daftarsiswayangtelahmengerjakan":
    if ($_SESSION[leveluser]=='admin'){
        ...
        echo "<br><div class='information msg'>
         Pilih Aksi <b>hapus mahahiswa</b> jika ingin mereset Siswa yang telah mengikuti ujian.<br>
        ...
        while ($t=mysql_fetch_array($siswa_yangmengerjakan2)){
            ...
            echo "<tr><td>$no</td>                      
                      ...
                      if ($t[dikoreksi]=='B'){
                          ...
                          echo "
                          <td><a href=$aksi?module=quiz&act=editsiswayangtelahmengerjakan&id=$t[id]&id_siswa=$s[id_siswa]&id_tq=$_GET[id]>hapus mahahiswa</a> |
        ...
        echo "</table>";
        echo "<br><input class='btn btn-info' type=button value=Kembali onclick=\"window.location.href='?module=quiz';\">";
        
        }else{
            echo "<script>window.alert('Belum ada mahahiswa yang mengikuti ujian.');
    ...
    elseif ($_SESSION[leveluser]=='dosen'){
        ...
        $siswa_yangmengerjakan2 = mysql_query("SELECT * FROM siswa_sudah_mengerjakan WHERE id_tq = '$_GET[id]'");
        echo "<br><table id='table1' class='table table-condensed table-bordered table-hover'><thead>
            ...
            echo "<tr><td>$no</td>
                      ...
                      if ($t[dikoreksi]=='B'){
                          ...
                          echo "<td>Jawaban soal essay <b>sudah di koreksi</b><br>Nilai Tugas/Quiz Pilihan Ganda : $n2[persentase]<br>
                                                         Nilai Tugas/Quiz Essay: $n[nilai]</td>";
                          echo "
                          <td><a href=javascript:confirmdelete('administrator/$aksi?module=quiz&act=editsiswayangtelahmengerjakan&id=$t[id]&id_siswa=$s[id_siswa]&id_tq=$_GET[id]')>hapus mahahiswa</a> |
                          <a href=?module=quiz&act=editkoreksi&id=$t[id_tq]&id_siswa=$s[id_siswa]>Edit Koreksi</a></td></tr>";
                      }
            $no++;
        }
...
case "hasilkoreksi":
    ...
    elseif ($_SESSION[leveluser]=='dosen'){
        echo "<form method=POST action='administrator/$aksi?module=quiz&act=inputnilai'><fieldset>
             <legend>Koreksi Nilai</legend>
             <dl class='inline'>";
        $jum_soal = $_POST['jumlah_soal'];

                    echo "<table id='table1' class='table table-condensed table-bordered table-hover'><thead>
                          <input type=hidden name=id_tq value='$_POST[id_topik]'>
                          <input type=hidden name=id_siswa value='$_POST[id_siswa]'>";
                    echo "<tr><th>No Soal</th><th>Jawaban</th><th>Nilai</th></tr></thead>";
                    for ($i=1; $i<=$jum_soal; $i++){
...
case "tambahtopikquiz":
    ...
    elseif ($_SESSION[leveluser]=='dosen'){
    echo "<form name='form_topik' method=POST action='administrator/$aksi?module=quiz&act=input_topikquiz'>
    <fieldset>
    <legend>Tambah Topik</legend>
    <dl class='inline'>
    <dt><label>Judul</label></dt>               <dd> <input type=text name='judul' size='50'></dd>
    <dt><label>Kelas</label></dt>               <dd> <select name='id_kelas' onChange='showpel_pengajar()'>
                                          <option value=''>-pilih-</option>";
                                          $pilih= mysql_query("SELECT DISTINCT id_kelas FROM mata_pelajaran WHERE id_pengajar ='$_SESSION[idsiswa]'");
    ...
    <dt><label>Terbit</label></dt>             <dd> <label><input type=radio name='terbit' value='Y'>Y</input></label>
          ...
          <p><input class='btn btn-info' type=submit value=Simpan>
          <input class='btn btn-info' type=button value=Batal onclick=self.history.back()></p>
...
case "edittopikquiz":
    ...
    echo "<form name='form_topik' method=POST action='administrator/$aksi?module=quiz&act=edit_topikquiz'>
    <input type=hidden name=id value='$t[id_tq]'>
    <fieldset>
    <legend>Edit Topik</legend>
    <dl class='inline'>
...
case "daftartopik":
    if ($_SESSION[leveluser]=='siswa'){
        ...
        if (!empty($cek_topik)){
            echo"<br><b class='judul'>Daftar Tugas / Quiz MataKuliah $data_mapel[nama]</b><br><p class='garisbawah'></p>
              <table class='table table-condensed table-bordered table-hover'>
              <tr><th>No</th><th>Deskripsi Tugas/Quiz </th><th></th></tr>";
              $no=1;
              while($t=mysql_fetch_array($topik)){
                  $tgl_posting   = tgl_indo($t[tgl_buat]);
                  $pengajar =  mysql_query("SELECT * FROM siswa WHERE id_siswa = '$t[pembuat]'");
                  ...
                  echo"<tr><td rowspan=6>$no</td><td><b>Judul</b></td><td><b>: $t[judul]</b></td></tr>
                       ...
                       echo"<tr><td><b>Waktu Pengerjaan</b></td><td><b>: $waktu menit</b></td></tr>
                            ...
                            <tr><td></td><td><input type=button class='btn btn-small btn-info' value='Kerjakan Tugas/Quiz'
                       onclick=\"window.location.href='?module=quiz&act=infokerjakan&id=$t[id_tq]';\"></td></tr>";
              $no++;
              }
              echo"</table>
                    <p class='garisbawah'></p><input type=button class='btn btn-small btn-info' value='Kembali' onclick=self.history.back()>";
        }else{
            echo "<script>window.alert('Belum ada Tugas atau Quiz di MataKuliah ini.');
                    window.location=(href='media.php?module=quiz')</script>";
        }
    }
    break;
...
case "daftarnilai":
    if ($_SESSION[leveluser]=='siswa'){
        ...
        if (!empty($cek_topik)){
            echo"<br><b class='judul'>Daftar Nilai Tugas / Quiz MataKuliah $data_mapel[nama]</b><br><p class='garisbawah'></p>
              <table class='table table-condensed table-bordered table-hover'>
              <tr><th>No</th><th>Deskripsi Tugas/Quiz </th><th></th></tr>";
              $no=1;
               while($t=mysql_fetch_array($topik)){
                  ...
                  echo"<tr><td rowspan=6>$no</td><td><b>Judul</b></td><td><b>: $t[judul]</b></td></tr>
                       ...
                       echo"<tr><td><b>Waktu Pengerjaan</b></td><td><b>: $waktu menit</b></td></tr>
                            <tr><td><b>Info Soal/Quiz</b></td><td><b>: $t[info]</b></td></tr>
                            <tr><td></td><td><input type=button class='btn btn-small btn-info' value='Lihat Nilai'
            ...
            echo "<script>window.alert('Belum ada Tugas atau Quiz di MataKuliah ini, jadi tidak ada nilai.');
...
case "infokerjakan":
    if ($_SESSION[leveluser]=='siswa'){
        ...
        echo"<br><b class='judul'>Informasi</b><br><p class='garisbawah'></p>
            ...
            4. Jika mati lampu hubungi Pengajar MataKuliah terkait untuk bisa Ujian Kembali.</h3><br>";
        echo"<p class='garisbawah'></p>
            <input type=submit class='btn btn-small btn-info' value='Mulai Mengerjakan' onclick='window.location.reload()'>
            <input type=button class='btn btn-small btn-info' value='Kembali' onclick=self.history.back()>";
        }
        elseif ($data[hits] >= 1){
            echo"<br><b class='judul'>Informasi</b><br><p class='garisbawah'></p>";
            echo "<h3>Anda Sudah mengerjakan tugas / Quiz ini</h3>";
            echo "<p class='garisbawah'></p>
                <input type=button class='btn btn-small btn-info' value='Kembali'
...
case "buatquizesay":
    if ($_SESSION[leveluser]=='admin'){
        ...
        echo "<form method=POST action='administrator/$aksi?module=quiz&act=input_quizesay' enctype='multipart/form-data'>
        <input type=hidden name=id value='$_GET[id]'>
        <fieldset>
        <legend>Buat Tugas/Quiz Essay</legend>
        <dl class='inline'>
...
case "buatquizpilganda":
    if ($_SESSION[leveluser]=='admin'){
        ...
        echo"<form method=POST action='administrator/$aksi?module=quiz&act=input_quizpilganda' enctype='multipart/form-data'>";
        echo"<fieldset>
             <legend>Buat Tugas/Quiz Pilihan Ganda</legend>
             <dl class='inline'>
             <dt><label>Pertanyaan ".$jumlah." </label></dt> <dd><textarea name='pertanyaan' id='wysiwyg' cols='75' rows='3'></textarea></dd>
...
case "daftarquiz":
    if ($_SESSION[leveluser]=='admin'){
        ...
        echo "<form><fieldset>
          ...
          <p align=center'>
          <input class='btn btn-warning' type=button value='Daftar Quiz Esay' onclick=\"window.location.href='?module=daftarquizesay&act=daftarquizesay&id=$t[id_tq]';\">
          <input class='btn btn-danger' type=button value='Daftar Quiz Pilihan Ganda' onclick=\"window.location.href='?module=daftarquizpilganda&act=daftarquizpilganda&id=$t[id_tq]';\">
          </p>
          <br><input type=button class='btn btn-info' value=Kembali onclick=self.history.back()>
          </fieldset></form>";
    }
...
case "daftarquizesay":
    if ($_SESSION[leveluser]=='admin'){
        ...
        echo "<div class='information msg'><b>Daftar Soal Quiz Esay</b></div>
              ...
              <dt><label>MataKuliah</label></dt>  <dd>: $p[nama]</dd>
              </dl></fieldset></form>";
        echo "<input class='btn btn-info' type=button value='Tambah Quiz Esay' onclick=\"window.location.href='?module=quiz&act=buatquizesay&id=$jq[id_tq]';\">
              <input class='btn btn-info' type=button value=Kembali onclick=\"window.location.href='?module=quiz';\">";
    ...
    elseif ($_SESSION[leveluser]=='dosen'){
        ...
        echo "<input class='btn btn-info' type=button value='Tambah Quiz Esay' onclick=\"window.location.href='?module=buatquizesay&act=buatquizesay&id=$jq[id_tq]';\">
              <input class='btn btn-info' type=button value=Kembali onclick=\"window.location.href='?module=quiz';\">";
    ...
    while ($q=mysql_fetch_array($quiz)){
       ...
       echo "<tr><td>$no.</td>
             ...
             <td>$tgl_buat</td>
             <td><a href='?module=quiz&act=editquizesay&id=$q[id_quiz]&id_topik=$q[id_tq]' title='Edit'><img src='images/icons/edit.png' alt='Edit' /></a> |
                 <a href=javascript:confirmdelete('administrator/$aksi?module=quiz&act=hapusquizesay&id=$q[id_quiz]&id_topik=$q[id_tq]') title='Hapus' ><img src='images/icons/cross.png' alt='Delete' /></a>
                 </td></tr>";
      $no++;
    }
    echo "</table>";
...
case "daftarquizpilganda":
    if ($_SESSION[leveluser]=='admin'){
        ...
        echo "<input type=button class='btn btn-info' value='Tambah Quiz Pilihan ganda' onclick=\"window.location.href='?module=quiz&act=buatquizpilganda&id=$jq[id_tq]';\">
              <input type=button class='btn btn-info' value=Kembali onclick=\"window.location.href='?module=quiz';\">";
    ...
    echo "</table>";
    echo "<br><input type=button class='btn btn-info' value='Tambah Quiz Pilihan ganda' onclick=\"window.location.href='?module=quiz&act=buatquizpilganda&id=$jq[id_tq]';\">
              <input type=button class='btn btn-info' value=Kembali onclick=\"window.location.href='?module=quiz';\">";
    }else{
       ...
       echo "<br><br><table id='table1' class='table table-condensed table-bordered table-hover'>";
    $no=1;
    while ($q=mysql_fetch_array($quiz)){
       echo "<tr><td rowspan=8><b>$no.</b></td>
             <td><b>Pertanyaan</b></td><td>: $q[pertanyaan]</td></tr>";
		    ...
		    <div>
             ...
             <tr><td>Kunci Jawaban</td><td>: $q[kunci]</td></tr>
             <tr><td>Aksi</td><td>: <a href='?module=quiz&act=editquizpilganda&id=$q[id_quiz]&id_topik=$q[id_tq]' title='Edit'><img src='images/icons/edit.png' alt='Edit' /></a> |
                 <a href=javascript:confirmdelete('administrator/$aksi?module=quiz&act=hapusquizpilganda&id=$q[id_quiz]&id_topik=$q[id_tq]') title='Hapus'><img src='images/icons/cross.png' alt='Delete' /></a></td></tr>
             ";
      $no++;
    }
    echo "</table>";
...
case "editquizesay":
    if ($_SESSION[leveluser]=='admin'){
       ...
       echo "<form method=POST action='administrator/$aksi?module=quiz&act=edit_quizesay' enctype='multipart/form-data'>
        <input type=hidden name=id value='$q[id_quiz]'>
        <input type=hidden name=topik value='$_GET[id_topik]'>
        <fieldset>
         <legend>Edit Tugas/Quiz</legend>
...
case "editquizpilganda":
    ...
    elseif ($_SESSION[leveluser]=='dosen'){
         $quiz=mysql_query("SELECT * FROM quiz_pilganda WHERE id_quiz = '$_GET[id]'");
        $q = mysql_fetch_array($quiz);
        echo "<form method=POST action='administrator/$aksi?module=quiz&act=edit_quizpilganda' enctype='multipart/form-data'>
        <input type=hidden name=id value='$q[id_quiz]'>
        <input type=hidden name=topik value='$_GET[id_topik]'>
        <fieldset>
        <legend>Edit Tugas/Quiz Pilihan Ganda</legend>
